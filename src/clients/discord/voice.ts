...
export class VoiceManager extends EventEmitter {
  ...
  async debouncedProcessTranscription(userId: UUID, name: string, userName: string, channel: BaseGuildVoiceChannel) {
    ...
    this.transcriptionTimeout = setTimeout(async () => {
      ...
      try {
        await this.processTranscription(userId, channel.id, channel, name, userName);
  ...
  async handleUserStream(
    ...
    const processBuffer = async (buffer: Buffer) => {
      try {
        ...
        this.debouncedProcessTranscription(userId, name, userName, channel);
      } catch (error) {
  ...
  private async handleUserMessage(
    ...
    try {
      ...
      const context = composeContext({
        state,
        template:
          this.runtime.character.templates?.discordVoiceHandlerTemplate ||
  ...
  private async _shouldRespond(
    ...
    if (
      ...
      lowerMessage.includes(this.client.user?.tag.toLowerCase() as string) ||
  ...
  private async _shouldIgnore(message: Memory): Promise<boolean> {
    ...
    if (
      ...
      loseInterestWords.some((word) => (message.content as Content).text?.toLowerCase().includes(word))
    ) {
      return true;
    }
    ...
    if (
      ...
      ignoreWords.some((word) => (message.content as Content).text?.toLowerCase().includes(word))
    ) {
      return true;
    }
  ...
  async handleJoinChannelCommand(interaction: any) {
    try {
      ...
      const voiceChannel = interaction.guild.channels.cache.find(
        (channel: VoiceChannel) => channel.id === channelId && channel.type === ChannelType.GuildVoice
      );
