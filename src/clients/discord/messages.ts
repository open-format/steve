...
export class MessageManager {
  ...
  async handleMessage(message: DiscordMessage) {
    if (message.interaction || message.author.id === this.client.user?.id /* || message.author?.bot*/) {
      return;
    }
    ...
    if (
      this.runtime.character.clientConfig?.discord?.shouldIgnoreDirectMessages &&
    ...
    if (
      ...
      !this.runtime.character.clientConfig?.discord?.shouldRespondOnlyToMentions
    ) {
      ...
      const mentionedTeamMember = otherTeamMembers.find((id) => message.content.includes(`<@${id}>`));
    ...
    try {
      ...
      if (message.channel.type === ChannelType.GuildVoice) {
        ...
        const speechService = this.runtime.getService<ISpeechService>(ServiceType.SPEECH_GENERATION);
        if (!speechService) {
    ...
    if (this._isTeamLeader() && channelState.messages.length > 0) {
      ...
      if (!this._isRelevantToTeamMember(lastMessage.content.text || "", channelId)) {
        const recentTeamResponses = channelState.messages
          .slice(-3)
          .some((m) => m.userId !== this.client.user?.id && this._isTeamMember(m.userId));
