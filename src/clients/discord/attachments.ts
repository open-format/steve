...
export class AttachmentManager {
  ...
  async processAttachments(attachments: Collection<string, Attachment> | Attachment[]): Promise<Media[]> {
    ...
    const attachmentCollection =
      attachments instanceof Collection ? attachments : new Collection(attachments.map((att) => [att.id, att]));
  ...
  private async processPdfAttachment(attachment: Attachment): Promise<Media> {
    try {
      ...
      const text = await this.runtime.getService<IPdfService>(ServiceType.PDF).convertPdfToText(Buffer.from(pdfBuffer));
